...
class Searcher {
  ...
  deactivate() {
    if(this.isearchCommands)
      this.isearchCommands.dispose();
    if(this.isearchKeymaps)
      this.isearchKeymaps.dispose();
  }
  ...
  activateIsearch(forward) {
    ...
    if(this.isearchTile && !this.isearchWord)
      this.isearchWord = this.isearchLast;
    this.searchNext(this.isearchWord);
  }
  ...
  createRegExp(word) {
    const escaped = word.split('').map((c)=>c=='\\' ? '\\\\' : '['+c+']').join('');
  ...
  searchNext (word, silent) {
    ...
    if(!editor || editor.mini)
      return;
    const selections = editor.getSelectedBufferRanges().map(sel=>
      this.getSelections(editor, sel, word, silent));
  ...
  getSelections (editor, sel, word, silent) {
    ...
    const start = selectedText.length ^ this.isforward ? sel.start : sel.end;
    const targetRanges = [new Range(start, bufferEnd), new Range([0, 0], start)];
    if(!this.isforward) targetRanges.reverse();
    const selectedRange = targetRanges.reduce((selected, targetRange) =>
      this.updateselectedRange(selected, editor, re, targetRange), null);
  ...
  updateselectedRange(selected, editor, re, targetRange) {
    ...
    let newRange = null;
    const selector = ({range, stop}) => { newRange = range; stop(); };
    if(this.isforward)
      editor.scanInBufferRange(re, targetRange, selector);
    else
      editor.backwardsScanInBufferRange(re, targetRange, selector);
    return newRange;
  }
  ...
  search(e, forward, useRegex) {
    if (atom.config.get('transient-emacs').useLegacySearch)
      this.activateIsearch(forward);
    else {
      this.searchWithFindAndReplace(e, forward, useRegex);
    }
  }

  searchWithFindAndReplace(e, forward, useRegex) {
