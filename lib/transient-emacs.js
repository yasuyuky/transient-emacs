...
module.exports = {
  ...
  activate: state => {
    ...
    const addEditorEventListner = editor => {
      ...
      let listener = new DOMListener(atom.views.getView(editor));
      module.exports.eventListeners.push(
        listener.add('atom-text-editor', 'click', () => {
    ...
    module.exports.killring =
      state && state.killring ? atom.deserializers.deserialize(state.killring) : new KillRing([]);
  },
  ...
  serialize: () => ({ killring: module.exports.killring.serialize() }),
  ...
  showKillRing: () => module.exports.killring.show(item => module.exports.yankTexts(item.value)),
